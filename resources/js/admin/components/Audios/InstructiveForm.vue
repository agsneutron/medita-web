        ...
        <v-card class="invisible-scrollbar" style="height: auto">
            <v-card-title class="d-flex justify-content-center bg-purple py-5">
                ...
                <div class="d-flex text-white text-center py-0 mb-0">
                    ...
                    <div class="mx-2"> Cambiar Instructivo </div>
                </div>
            </v-card-title>
            <v-card-text>
                    ...
                    <v-container class="px-3rem">
                        <v-row justify="center" style="margin: 1rem 0rem;">
                            <v-col cols="4" class="p-0 dropzoneTitle">
                                <p class="m-0 pink-text"> Adjuntar Instructivo </p>
                            </v-col>
                            <v-col cols="8" class="py-0" style="text-align: center;">
                                <vue2-dropzone
                                    class="elDropzone"
                                    ref="instructiveDropzone"
                                    id="instructiveDropzone"
...
export default {
    ...
    methods: {
        ...
        dzUploadFile() {
            this.$refs.instructiveDropzone.removeAllFiles(true);
            let file = { size: 1234, name: "file.pdf"};
            let url = this.dataUpdate
            this.$refs.instructiveDropzone.manuallyAddFile(file, url);
        },
        dzMaxFilesExceeded(file) {
            this.alerta("Error al agregar archivo:","Ya hay un archivo adjunto");
            this.$refs.instructiveDropzone.removeAllFiles();
            // this.$refs.instructiveDropzone.removeFile(file);
        },
        dzRemovedFile() {
            this.url = null;
        ...
        submit() {
            if (this.url !== null) {
                this.loading = true;
                axios.post(route('admin.categories.instructive_upsert', {id: this.$route.params.id}), this.formData()).then(response => {
                    if (response.data.success) {
                        this.$swal(
                            {
                                title: !this.isUpdate ? '¡Instructivo agregado!' : '¡Instructivo modificado!',
                                text: !this.isUpdate ? 'El instructivo se ha agregado correctamente.' : 'El instructivo se ha cambiado correctamente.',
                                icon: 'success',
                                timer: 2000,
                                iconColor: '#62457F',
                                showConfirmButton: false,
