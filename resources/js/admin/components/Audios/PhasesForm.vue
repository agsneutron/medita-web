        ...
        <v-card class="invisible-scrollbar">
            <v-card-title class="d-flex justify-content-center bg-purple py-5">
                ...
                <div class="d-flex text-white text-center py-0 mb-0">
                    ...
                    <div class="mx-2">{{ isUpdate === false ? 'Agregar ' : 'Modificar ' }}</div>
                </div>
            </v-card-title>
            <v-card-text>
                <v-form ref="form" lazy-validation>
                    <v-container class="px-3rem">
                        <v-row justify="center" v-if="this.categoriasDePago.includes(categoryId)">
    ...
    data() {
        return {
            ...
            rules: {
                ...
                price: [v => !!v || 'El precio es requerido.'],
                // description: [v => !!v || 'La descripción  es requerida.'],
            },

            dropzoneOptions: {
                url: route('admin.categories.phase_add_file'),
        ...
        submit() {
            ...
            if( valid ){
                if (this.url_image !== null) {
                    this.loading = true;
                    axios.post(route('admin.categories.phase_upsert', {id: !this.isUpdate ? 'FAKE_ID' : this.dataUpdate.id}), this.formData()).then(response => {
                        if (response.data.success) {
                            this.$swal(
                                {
                                    title: !this.isUpdate ? '¡Fase agregada!' : '¡Fase modificada!',
                                    text: !this.isUpdate ? 'La fase se ha agregado correctamente.' : 'La fase se ha modificado correctamente.',
                                    icon: 'success',
                                    timer: 2000,
                                    iconColor: '#62457F',
