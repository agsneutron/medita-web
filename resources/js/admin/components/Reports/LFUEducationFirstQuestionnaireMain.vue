...
export default {
    name: "LFUEducationFirstQuestionnaireMain",
    ...
    methods: {
        ...
        async fetchData(preventSpinner = false) {
            ...
            try {
                const {data} = await axios.get(route('admin.reports.lfu_first_questionnaire'), {params});
        ...
        excelExport(preventSpinner = false){
            let params = this.prepareParams(preventSpinner);
            params['educational_level'] = this.educationalLevel;
            axios.get(route('admin.reports.export_lfuEducationFirstQuestionnaire'), {
                params,
                ...
                link.setAttribute('download', 'ReporteLFUEducationPrimerCuestionario.xlsx');
